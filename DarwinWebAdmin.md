...
| Identity/Admin Support | In Progress | Users, roles, permissions, and core admin identity flows exist; the users workspace now also exposes queue-style lifecycle filters, quick support actions, direct activation/password-reset/mobile remediation handoffs from lifecycle state, live failed-activation/failed-password-reset summary counts from the communication-audit lane, actionable support playbooks that jump straight into the exact queue/audit/mobile remediation lanes, and direct deep-links from summary cards, row identity cells, lifecycle badges, and active-state badges into the right remediation surface, so identity triage can start from the exact signal instead of only from filter buttons or the trailing row action cluster, but broader invite/activation/support depth still needs audit. |
