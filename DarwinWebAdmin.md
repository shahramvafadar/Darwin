...
| Communication Management | In Progress | Business communications now have HTMX-safe workspace/profile/audit flows, template and retry visibility matrices, flow-specific playbooks, safe test-email reruns, recipient-aware handoffs into invitation/user/business support surfaces, delivery age/latency/severity context, controlled generic retry for failed or pending invitation/activation/password-reset rows after safe target resolution, and first-class `repeated failures` / `prior success context` triage on top of prior-attempt delivery history for the same flow-recipient path; deeper delivery-log infrastructure, template CRUD, and broader replay remain near-term. |
| Shipping Operations | In Progress | Generic order shipment visibility exists, and WebAdmin now exposes both shipping-method configuration with queue filters/editable rate tiers and a cross-order shipment queue for packing, tracking, carrier-review, and return-follow-up work; deeper DHL carrier lifecycle and full RMA completion still remain near-term. |
...
- `In Progress`: platform communication management now supports operationally safe phase-1 email troubleshooting with controlled retry, prior-attempt context, and repeated-failure triage, but is not yet complete enough for a full template/delivery-log platform
...
- `Completed foundation`: site settings now also expose phase-1 SMS and WhatsApp test recipients, and the communication workspace surfaces whether each channel has a safe rollout target configured before multi-channel test-send tooling exists
