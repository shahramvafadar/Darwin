...
| Identity/Admin Support | In Progress | Users, roles, permissions, and core admin identity flows exist; the users workspace now also exposes queue-style lifecycle filters, quick support actions, and direct activation/password-reset/mobile remediation handoffs from both summary cards and lifecycle badges for unconfirmed, locked, inactive, and mobile-linked accounts, but broader invite/activation/support depth still needs audit. |
