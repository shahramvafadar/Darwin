...
- `Completed foundation`: `/catalog` now also exposes a page-local review-priority lens for offers-first and base-first windows, so browse can inspect the currently loaded assortment from a clearer commercial review order without pretending true backend facets already exist
- `Completed foundation`: `/cms` now also exposes a page-local discovery-state lens for discovery-ready pages versus pages still missing metadata context, so public content review is more actionable without pretending true backend CMS search already exists
- `Completed foundation`: `/cms` now also exposes a page-local review-priority lens for featured order, title order, ready-first, and attention-first windows, so public content review can move between stronger discovery candidates and metadata debt without pretending true backend CMS search already exists
