...
- [x] Mobile profile save flow now uses `Result`-based update responses in shared profile service and maps optimistic-concurrency conflicts to explicit localized retry guidance (Consumer + Business), while avoiding raw exception leakage in Business profile UI.
