...
- `In Progress`: `/loyalty/[businessId]` now also renders a real QR image from the canonical short-lived scan token so browser-prepared scan sessions are immediately usable for staff-side scanning
...
- `Later-phase only`: camera/scanner handoff, interactive map-pan/zoom flows, push-heavy engagement surfaces, and deeper loyalty gamification outside the current contract set
...
- `In Progress`: `Darwin.Web` now has a resource-bundle localization foundation under `src/Darwin.Web/src/localization/resources`, and shared shell/catalog/storefront-commerce copy is moving onto that path so future languages can be added without reworking component structure
...
- `Later-phase only`: translated CMS/page content operations, localized SEO metadata management, migration of the remaining member/account/loyalty screens onto resource bundles, and additive languages beyond the current de/en baseline
