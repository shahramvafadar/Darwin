...
3. Confirm extensions can be created by the migration user:
   - `citext`
   - `pg_trgm`
4. Run a pending-model check from the same application version:
...
docker exec darwin-postgres psql -U darwin -d darwin_dev -c "select count(*) from information_schema.columns where udt_name = 'citext';"
docker exec darwin-postgres psql -U darwin -d darwin_dev -c "select count(*) from information_schema.columns where udt_name = 'jsonb';"
docker exec darwin-postgres psql -U darwin -d darwin_dev -c "select count(*) from pg_indexes where indexname like 'IX_PG_%_JsonbGin';"
docker exec darwin-postgres psql -U darwin -d darwin_dev -c "select count(*) from pg_indexes where indexname in ('IX_PG_EventLogs_PropertiesJson_Trgm', 'IX_PG_ProviderCallbackInboxMessages_PayloadJson_Trgm', 'IX_PG_Businesses_AdminTextOverridesJson_Trgm');"
docker exec darwin-postgres psql -U darwin -d darwin_dev -c "select count(*), count(*) filter (where not convalidated) from pg_constraint where conname like 'CK_PG_%_ValidJson';"
```
