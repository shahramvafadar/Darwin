...
- Phase-3 increment delivered: subscription settings entry point with portal configuration diagnostics (missing/invalid/non-HTTPS/host-allowlist), HTTPS-validated launch, copy-url support, server-backed read-only subscription status snapshot, and cancel-at-period-end toggle for operator self-service/troubleshooting.
