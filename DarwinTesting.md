...
| 4 | Loyalty scan journey test pack | In Progress | Baseline auth-guard tests exist (`my/businesses`, `my/accounts`, `my/promotions`, `my/timeline`, `scan/prepare`, `scan/process`, `scan/confirm-accrual`, `scan/confirm-redemption` anonymous => 401); next add authenticated end-to-end prepare/process/confirm flows |
...
- Expanded identity baseline with anonymous-guard coverage for `POST /api/v1/auth/logout-all` (`401 Unauthorized`).
