...
- [ ] Add contract serialization compatibility tests for mobile-critical DTOs. Baseline coverage plus expanded Loyalty timeline/promotions, Profile concurrency payload, Push registration payloads, Business discovery compatibility checks, business campaign/reward configuration property-name compatibility checks (including optional content fields such as subtitle/body/media/landing URLs), and campaign create/update/list (including item `businessId`/`rowVersion` and explicit create/update schedule field assertions plus targeting/payload JSON string assertions) + reward-tier configuration/delete/mutation payload serialize/deserialize compatibility checks (including tier value/type/self-redemption field assertions) are implemented in code (`Darwin.Contracts.Tests` + `Darwin.Tests.Unit` + `Darwin.WebApi.Tests` mapper suites); pending CLI/CI execution evidence.
