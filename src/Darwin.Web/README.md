...
- home composition now also surfaces explicit promotion lanes for hero offers, value offers, live offer momentum, and steady base assortment, so storefront entry can jump directly into the strongest merchandising windows instead of only showing generic offer and campaign teasers
...
- public API cache policy now also exposes one canonical cache-identity helper for normalized path, revalidate window, and key/path tags, so public fetch wiring no longer has to rebuild those cache-identity pieces in parallel
...
- public JSON fetch wiring now also runs through one canonical response parser for success, response-failure, and invalid-payload branches, so storefront degradation handling is less likely to drift behind inline JSON parsing paths
- public JSON fetch execution now also runs through one canonical request executor for fetch, parse, network-failure, and failure-logging branches, so storefront API wiring is less likely to drift behind inline request orchestration
- public JSON fetch planning now also runs through one canonical WebApi execution-context helper, so request URL, normalized path, and cache-aware init stay aligned instead of being rebuilt inline in the storefront fetch path
- public JSON parsed-response shaping now also runs through canonical success/failure helpers, so response handling is less likely to drift behind ad hoc parsed-result object construction
...
- public JSON response-bound handling now also flows through one canonical response context for diagnostics, success, and degraded outcomes, so `key/path/response` execution state is less likely to drift behind repeated inline fetch branches
...
- the security self-service route now also includes the shared self-service composition window with an explicit current-task, next-step, and storefront route map, so profile, preferences, addresses, and security all guide the member through one consistent self-service journey instead of stopping at forms and side rails alone.
- checkout now also includes a dedicated composition window with an explicit current task, next conversion step, and route map into cart, account continuity, CMS, and catalog, so the main pre-purchase surface no longer behaves like an isolated form route before order placement.
- sign-in, register, activation, and password recovery now also share one public-auth composition window with explicit current-route, next-account-step, and storefront route mapping, so the core auth/recovery family no longer stops at isolated public forms plus continuation rails alone.
...
- public auth and protected member-entry route-health summaries now also carry compact workflow footprints, so entry-route diagnostics can read route/session/cart lane state directly instead of only isolated storefront counts.
- account, cart, checkout, and confirmation route-health summaries now also carry compact workflow footprints, so entry and conversion diagnostics can read session/address/order/invoice lane state directly instead of only isolated counts.
- catalog detail, CMS detail, product follow-up, and CMS route-health summaries now also carry compact workflow footprints, so drill-in diagnostics can read product/page and related-follow-up state directly instead of only isolated has/count fields.
- catalog and CMS index route-health summaries now also carry compact browse workflow footprints, so discovery diagnostics can read mode, matching-set state, visible window, and review or promotion context directly instead of only isolated counters.
- Home and shared commerce route-health summaries now also carry compact workflow footprints, so storefront entry and conversion diagnostics can read session, cart, order, invoice, confirmation, and merchandising-lane state directly instead of only isolated counters.
- home discovery, confirmation result, and paged member-collection summaries now also carry compact workflow footprints, so storefront entry, after-purchase, and paged collection diagnostics can read state directly instead of only isolated counters.
- localized discovery inventory, alternates, sitemap, and SEO metadata summaries now also carry compact workflow footprints, so multilingual and SEO diagnostics can read state directly instead of reconstructing it from several separate footprint fields.
- public CMS search windows and catalog review/search windows now use path-aware cache revalidation instead of key-only timing, so storefront performance tuning can distinguish stable discovery feeds from more volatile search and merchandising slices.
- public CMS expanded page-set windows and catalog category or full-set browse windows now also use path-aware cache revalidation, so discovery performance can keep volatile review and merchandising surfaces fresher than the baseline stable feeds.
- member dashboard, editor, and collection route-health now carry merchandising-aware workflow footprints, and direct route-health coverage now validates those richer member workflow summaries so protected portal diagnostics do not drift behind count-only expectations.
- Home now renders promotion lanes as a real merchandising card board instead of a status list, so hero, value, live, and base windows read like first-class storefront campaigns and align with the other main promotion surfaces.
- direct coverage now validates that Home renders promotion lanes as a card-grid merchandising board while keeping route-map and readiness web-part shapes intact, so the main storefront entry is less likely to drift back into a status-only lane surface.
- product detail now also includes a dedicated storefront support window for CMS follow-up, catalog browse lanes, live product follow-up, and cart or checkout continuity, so drilled-in catalog routes no longer rely only on composition and related-product blocks for storefront continuation.
...
- Account and commerce storefront windows now carry explicit cart/checkout continuity sections, so self-service and conversion routes no longer rely only on campaign or composition follow-up for purchase continuation.
- Route-health for account and commerce surfaces now carries explicit storefront-support footprints, so cart/checkout continuity and support-window state are operationally visible alongside promotion lanes.
- Member dashboard storefront continuity is now integrated into the shared member storefront window, so dashboard follow-up keeps cart and checkout support inside the same storefront surface instead of a separate side panel.
- Order and invoice detail routes now carry live cart/checkout continuity inside the shared member storefront window, with storefront cart state passed through from the protected route context.
- Member order and invoice detail diagnostics now emit a compact storefront-support footprint so detail routes expose CMS/category/product/cart continuity operationally, not only in UI.
- Orders and invoices history routes now carry live cart/checkout continuity inside the shared member storefront window, with storefront cart state passed through from protected route context.
- Public discovery cache policy now shortens revalidate windows for deeper CMS and catalog pagination, keeping multi-page browse results fresher than stable first-page feeds.
- Catalog and CMS storefront support windows now render explicit promotion-lane boards, so browse/content support surfaces expose hero, value, live, and base merchandising windows directly instead of only generic product follow-up.

- Catalog/CMS route-health now carries explicit browse/support workflow footprints for new storefront support surfaces, including merchandising lanes and cart continuity.

- Product detail storefront support now includes an explicit promotion-lane board, so drill-in merchandising parity matches catalog and CMS support surfaces.

- Guest confirmation now includes an explicit promotion-lane board so post-purchase merchandising parity matches the main commerce surfaces.

- Confirmation route-health now reports explicit follow-up merchandising footprints so guest after-purchase lanes are visible in diagnostics, not only in the UI.

- Added direct render coverage for guest confirmation merchandising so the after-purchase promotion-lane board does not rely only on build-time safety.

- Direct coverage for member/account/commerce storefront windows now also asserts promotion-lane boards, not only cart continuity.

- Entry and home route-health summaries now carry explicit storefront support footprints so CMS/category/product/cart continuity is visible directly in diagnostics.

- Cart, checkout, and confirmation route-health now expose explicit storefront support footprints so commerce continuity is easier to trace operationally.
- Catalog browse cache policy now also shortens revalidate windows for deeper paginated product results, so page-two-plus discovery stays fresher instead of sharing the slower first-page browse timing.
- Direct render coverage for catalog storefront support now also asserts hero/value/live/base promotion lanes, so browse-support merchandising no longer relies only on build-time safety.
- Product and CMS detail loaders now also emit explicit support-workflow footprints through dedicated helpers with direct unit coverage, so drill-in diagnostics can read related and review follow-up state without relying only on route-health rollups.
- Catalog and CMS index loaders now also emit explicit support-workflow footprints through dedicated helpers with direct unit coverage, so browse-route diagnostics can read CMS/category/product/cart support state without relying only on route-health rollups.
- Member dashboard, editor, collection, and detail route loaders now also emit explicit storefront-support footprints through a dedicated helper with direct unit coverage, so protected route diagnostics can read CMS/category/product/cart support state beyond route-health-only rollups.
- Account and commerce route loaders now also emit explicit storefront-support footprints through dedicated helpers with direct unit coverage, so entry and conversion diagnostics can read CMS/category/product/cart support state beyond route-health-only rollups.
- Public auth route/page loaders plus protected member-entry context now also emit explicit storefront-support footprints through dedicated helpers with direct unit coverage, so auth and entry diagnostics can read CMS/category/product/cart continuity beyond route-health-only rollups.
- Route-health expectations now also assert the newer storefront-support footprints for home, auth, account, member-entry, and commerce flows, so those diagnostics do not rely only on helper-level tests.
- Added a dedicated storefront-support helper plus direct unit coverage for the Home route loader, so Home success diagnostics expose CMS/category/product/cart continuity directly instead of relying only on route-health rollups.
- Added direct render coverage for PublicAuthContinuation so auth-entry promotion lanes plus cart/CMS/catalog continuity do not rely only on page-level build safety.
- Tightened path-aware cache windows for large and paged catalog-category feeds, so category-heavy discovery and support surfaces refresh faster than the stable long-lived baseline.
- Added a dedicated storefront-support helper plus direct unit coverage for Home discovery, so success diagnostics for the public storefront entry carry CMS/category/product/cart continuity directly instead of relying only on route-health rollups.
- Added direct render coverage for the public account hub page so its action-center, cart continuity, CMS/catalog follow-up, and promotion-lane board do not rely only on inner-window or build-time safety.
- Added direct render coverage for the public CMS index page so composition, storefront support, promotion lanes, and cart continuity on `/cms` do not rely only on inner-window tests.
- Added direct render coverage for the public catalog page so composition, storefront support, promotion-lane follow-up, and cart continuity on `/catalog` do not rely only on inner-window tests.
- Added direct render coverage for the product detail page so composition, storefront support, promotion-lane follow-up, and cart continuity on `/catalog/[slug]` do not rely only on inner-window tests.
- Added direct render coverage for the CMS detail page so composition, storefront support, promotion-lane follow-up, and cart continuity on `/cms/[slug]` do not rely only on inner-window tests.
- Added direct render coverage for the member dashboard page so storefront support, promotion lanes, and cart/checkout continuity on the protected overview route do not rely only on inner-window tests.
- Added direct render coverage for the checkout page so conversion support, promotion lanes, and cart/account continuity on `/checkout` do not rely only on inner-window tests.
- Added direct render coverage for the orders history page so storefront support, promotion lanes, and cart/checkout continuity on `/orders` do not rely only on inner-window tests.
- Added direct render coverage for the invoices history page so storefront support, promotion lanes, and cart/checkout continuity on `/invoices` do not rely only on inner-window tests.
- Added direct render coverage for the order detail page so storefront support, promotion lanes, and cart/checkout continuity on `/orders/[id]` do not rely only on inner-window tests.
- Added direct render coverage for sign-in, register, activation, and password-recovery pages so the main public-auth family no longer relies only on continuation/composition window tests.
- Added direct render coverage for catalog and CMS campaign windows so those promotion-lane campaign surfaces do not rely only on surrounding page or support-window tests.
- Storefront readiness audit now confirms that local development seed covers the core front-office graph for realistic testing, including demo users with default addresses, catalog products and categories, CMS pages, campaigns, shipping methods and rates, promotions such as `WELCOME10`, sample carts, and seeded orders with payments/shipments/refunds.
- Storefront flow audit now confirms that the non-loyalty front-office path is end-to-end in code: product display, add-to-cart, cart mutation, coupon apply/clear, checkout-intent creation, order placement, hosted payment handoff creation, payment completion reconciliation, and confirmation rendering are all wired through Darwin.Web plus Darwin.WebApi.
- Added integration smoke coverage for the seeded storefront baseline and anonymous commerce path, including seeded coupon application, so discovery seed presence plus cart -> coupon -> checkout -> payment -> confirmation flow no longer depend only on manual audit.
- Local hosted-checkout development readiness is now complete: Darwin.WebApi development settings hand payment intents back to the local web app, and Darwin.Web now exposes /mock-checkout to drive explicit success, cancellation, and failure returns into confirmation-finalize reconciliation.
- Added direct render coverage for the local `/mock-checkout` hosted-payment surface, so success, cancellation, and failure reconciliation links do not rely only on lint/build safety.
- Added direct unit coverage for the `/mock-checkout` route helpers, so bounded search parsing plus success/cancellation/failure finalize URL building do not rely only on component rendering and build safety.
