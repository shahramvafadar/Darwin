...
- the CMS index now also offers a page-local discovery-state lens for discovery-ready pages versus pages that still need metadata attention, so public content review is more actionable without pretending real backend CMS search already exists
- the CMS index now also offers a page-local review-priority lens for featured order, title order, ready-first, and attention-first windows, so public content review can move between stronger discovery candidates and metadata debt without pretending real backend CMS search already exists
...
- `/catalog` now also includes a page-local review-priority lens for `offers-first` and `base-first`, so browse can inspect the currently loaded assortment from a clearer commercial review order without pretending real backend facets already exist
